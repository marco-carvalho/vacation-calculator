...
const CheckboxGroup: React.FC<CheckboxGroupProps> = ({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-3 p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700/50">
        {options.map((option) => (
          <label
            key={option.value}
  ...
  const handleCalculate = (): void => {
    ...
    const checkForOverlaps = (
      ...
      for (const period of existingPeriods) {
        if (
          bufferStartDate <= period.endDate &&
          bufferEndDate >= period.startDate
        ) {
          return true;
        }
      }
    ...
    const findBestAvailablePeriods = (
      ...
      candidatePeriods.sort((a, b) => {
        ...
        const diffA = Math.abs(
          a.period.usedStartDate.getTime() - startDate.getTime(),
        );
        const diffB = Math.abs(
          b.period.usedStartDate.getTime() - startDate.getTime(),
        );
        return diffA - diffB;
      });
      ...
      <div className="mx-auto py-6 space-y-6 container">
        ...
        {isDrawerVisible && (
          <>
            ...
            />
            <aside
              className={`fixed top-0 right-0 h-dvh w-full max-w-xs bg-white dark:bg-gray-800 shadow-xl z-30 flex flex-col overflow-hidden transition-transform duration-300 ease-out dark:shadow-gray-950/50 ${
